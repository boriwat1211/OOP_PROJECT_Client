...
class Data_Set extends React.Component
{
    ...
    {
        const b_path = "http://localhost:9000/Send_data/D:/OOP_project/"+path;
        await axios(
            {
                method: "get",
                url: b_path,  
                            ...
                            axios(
                                {
                                    ...
                                    params:
                                    {
                                        path :"D:/OOP_project/"+this.state.current_path+"/"+b_name
                                    }
                                }).then((res) =>this.setState({image:res.data})).then(()=>this.setState({image_check:true}))
                            console.log("D:/OOP_project/"+this.state.current_path);
                    }}>{b_name}</AwesomeButton>; 
                    arr.push(temp);
                }
            }
                        ...
                        <SortableTree
                            ...
                            return { 
                                onDoubleClick: () => {
                                    ...
                                    console.log("D:/OOP_project/"+node.subtitle.toString())
                                    axios(
                                    {
                                        method :'get',
                                        url: "http://localhost:9000/Send_data/image",
                                        params:
                                        {
                                            path :"D:/OOP_project/"+node.subtitle.toString()
                                        }
